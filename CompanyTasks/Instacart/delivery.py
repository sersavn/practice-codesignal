'''
Instacart customers are able to set the delivery window during which they want to receive their groceries.
...
Before taking an order a shopper wants to ensure they will make it in time.
...
which means that he will need (300 + 200) / 40 + 5 = 17.5 minutes to fulfill the order.
This will leave him with 20 - 17.5 = 2.5 idle minutes, so he shouldn't take the order.
The second shopper is 600 m away from the store, his speed is 40 m/min, and he will spend 10 minutes in the store,
which means it will take him (600 + 200) / 40 + 10 = 30 minutes to fulfill the order.
...
def delivery(order, shoppers):
    ...
    for shopper in shoppers:
        deliveryt = (shopper[0] + order[0])/shopper[1]+shopper[2]
        if order[1] <= deliveryt <= (order[2]+order[1]):
